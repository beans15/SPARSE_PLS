    ...
    **kwargs : dict
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray) -> "SparsePLS":
        ...
        if np.any(pd.isnull(X)) or np.any(pd.isnull(Y)):
            raise ValueError(
                "Input data X and Y must not contain missing values. Please handle missing data before fitting."
            )
        ...
        # Ensure Y is 2D for multi-output handling
        if Y.ndim == 1:
            Y = Y.reshape(-1, 1)

        if self.scale:
            self._x_scaler = DataPreprocessor(
                method=self.scale_method, **self.scaler_kwargs
            )
            self._y_scaler = DataPreprocessor(
                method=self.scale_method, **self.scaler_kwargs
            )
        ...
        for k in range(self.n_components):
            ...
            logger.info(
                f"Component {k + 1}: Norm of t = {t_norm:.4f}, Deflation step completed."
            )

        self.coef_ = (
            self.x_weights_
            @ self._regularized_pinv(self.x_loadings_.T @ self.x_weights_)
            @ self.y_loadings_.T
        )
    ...
    def _log_iteration_state(
        self, iteration: int, w: np.ndarray, c: np.ndarray, change_in_weights: float
    ) -> None:
        """
    ...
    def export_estimator(self, filepath: str, format: str = "pickle") -> None:
        ...
        if format == "pickle":
            ...
            raise ValueError(
                "Unsupported format. Only 'pickle' is currently supported."
            )
    ...
    def get_selected_feature_names(self) -> list:
        ...
        if hasattr(self, "feature_names_in_") and self.feature_names_in_ is not None:
            ...
            return [
                str(self.feature_names_in_[i])
                for i in self.selected_variables_
    ...
    def optimize_parameters(
        ...
        if verbose > 0:
            logger.info(
                f"Fitting {len(param_combinations)} parameter combinations across {cv} folds..."
            )
        ...
        for param_name in param_grid.keys():
            cv_results_data[f"param_{param_name}"] = [
                r["params"].get(param_name) for r in results
            ]

        # Add detailed scores if requested
        if verbose > 1:
            for i, r in enumerate(results):
                cv_results_data[f"split{i}_score"] = [
                    res["scores"][i] if i < len(res["scores"]) else np.nan
                    for res in results
                ]
